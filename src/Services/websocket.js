export const WsSubscribers = {
  ...
  unsubscribe: function (channels, events, callback) {
    if (typeof channels === "string") {
      channels = [channels];
    }
    if (typeof events === "string") {
    ...
    channels.forEach(function (channel) {
      events.forEach(function (event) {
        if (
          ...
          // Find and remove the callback from the subscribers list
          const index =
            WsSubscribers.__subscribers[channel][event].indexOf(callback);
          if (index !== -1) {
            WsSubscribers.__subscribers[channel][event].splice(index, 1);
          }
        }
      });
    });
